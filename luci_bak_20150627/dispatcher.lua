...
function errorpage(errcode, message)
    if tonumber(errcode) then
        luci.http.status(tonumber(errcode), "Not Found")
        require("luci.template")
        if not luci.util.copcall(luci.template.render, "error404") then
            luci.http.prepare_content("text/plain")
            luci.http.write(message)
...
function empower(lan,wan,admin)
    ...
    if not XQFunction.isStrNil(mac) then
        ...
        if not XQSysUtil.setMacFilter(mac,nil,wan,admin) then
...
function loginAuthenFailed()
    ...
    XQPushHelper.push_request_lua(payload)
end
...
function dispatch(request)
    ...
    if not XQSysUtil.getInitInfo() then
        ...
        local clang = XQCountryCode.LANGUAGE[ccode]
        if clang ~= lang and conf.languages[clang] then
            lang = clang
            XQSysUtil.setLang(lang)
        end
    else
        local isrelease = XQSysUtil.getChannel() == "release"
        if not isrelease and lang ~= "zh_cn" then
            lang = "zh_cn"
            XQSysUtil.setLang(lang)
        end
    end
    ...
    if uri and uri:match("/cgi%-bin/luci/web/ng%-fault") then
        local errcode = luci.http.formvalue("ngcode")
        errorpage(errcode, "This is a test!")
        return
    end
